...
__global__ void init_t1_synapses(struct Synapse *d_t1_synapses, size_t t1_pitch, size_t pitch, int *d_t1_neuron_outputs, int *d_neuron_outputs, int *d_brain_inputs, curandState_t *d_curand_state){
    int neuron = blockIdx.x*blockDim.x + threadIdx.x;
    
    if(neuron < NUM_T1_NEURONS){
        printf("neuron: %d  ", neuron);
        //set randomly 3/13 of the outputs to 1
        d_t1_neuron_outputs[neuron] = curand(&d_curand_state[neuron]) % 13 / 10;
        ...
        for(int synapse = 0; synapse < NUM_SYNAPSES_PER_NEURON; synapse++){
            ...
            //printf("neuron %d synapse: %d weight %.2f activity: %.2f\n", neuron, synapse, neuron_array[synapse].weight, neuron_array[synapse].activity);
            //random input
            int rand_input = curand(&d_curand_state[neuron]) % (NUM_NEURONS + NUM_T1_NEURONS + NUM_INPUTS);
            if(rand_input < NUM_NEURONS){
