...
__global__ void init_random_seed(unsigned int seed, curandState_t curand_state) {
...
__global__ void init_synapses(struct Synapse *d_synapses, size_t pitch, int *d_neuron_outputs, int *d_brain_inputs, curandState_t curand_state){
    int synapse = blockIdx.x*blockDim.x + threadIdx.x;
    int neuron = blockIdx.y*blockDim.y + threadIdx.y;

    skipahead(synapse * (neuron + 1), &curand_state);
    float new_weight = curand_uniform(&curand_state);
    
    struct Synapse *neuron_array = (struct Synapse *) ((char*)d_synapses + neuron * pitch);
    neuron_array[synapse].weight = new_weight;
    
    int rand_input = curand(&curand_state) % (NUM_NEURONS + NUM_INPUTS);
